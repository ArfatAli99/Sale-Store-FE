import { PagerService } from './../../../_services/pager.service';
import { Component, OnInit } from '@angular/core';

import { CommonserviceService } from '../../../commonservice.service';
import { environment } from '../../../../environments/environment';
import Swal from 'sweetalert2';

@Component({
	selector: 'app-article-listing',
	templateUrl: './article-listing.component.html',
	styleUrls: ['./article-listing.component.scss']
})
export class ArticleListingComponent implements OnInit {

	articlesListDetails = [];
	pageOffset: number = 1;
	search_text: string = '';
	limitPerPage = environment.articelListingLimit;
	countTotalPage: number;
	pager: any = {};
	isPageSetFirstTime: Boolean = false;

	/**
   * Creates an instance.
   * input: @param commonservice 
   * @param pagerService
   * created by : Biswajit
   */
	constructor(
		private commonService: CommonserviceService,
		private pagerService: PagerService,
		// public dialog: MatDialog
	) {
		this.setDefaultPaginationParameter();
	}

	/**
   * ngOnInit
   * method: life cycle hook
   * purpose: initialize
   * created by : Biswajit
   */
    ngOnInit() {
		this.articlesListng();
	}

	/**
 * Sets default pagination parameter
 * created by : Sourav
 */
	setDefaultPaginationParameter() {
		this.pageOffset = history.state['pageState'] ? history.state['pageState'] : 1;
		this.countTotalPage = 0;
		this.isPageSetFirstTime = false;
	}

	/**
 * Users status active deactive
 * @param index
 * @param status
 * created by: Sourav
 */
	articleApprovedDisapproved(index, status) {
		// let msg = "Do you want to disapprove?";

		// if (this.articlesListDetails[index].is_approved == 0) {
		// 	msg = "Do you want to approve?";
		// }
		// this.commonSwalAlert(msg, 'warning', true, true, 'Cancel', 'Confirm', false, '', index);

		let msg = "";

		if (status == 0) {
			msg = "Do you want to disapprove?";
		} else if (status == 1) {
			msg = "Do you want to approve?";
		}

		this.commonSwalAlert(msg, 'warning', true, true, 'Cancel', 'Confirm', false, '', index);
	}


	/**
	 * Commons swal alert
	 * @param title_txt
	 * @param alert_type
	 * @param show_cancel_btn
	 * @param show_confirm_btn
	 * @param cancel_btn_txt
	 * @param confirm_btn_txt
	 * @param outsite_clickable
	 * @param msg
	 * @param index
	 */
	commonSwalAlert(title_txt: string, alert_type: any, show_cancel_btn: any, show_confirm_btn: any,
		cancel_btn_txt: string, confirm_btn_txt: string, outsite_clickable: any, msg: string, index) {

		Swal.fire({
			title: title_txt,
			type: alert_type,
			showConfirmButton: show_confirm_btn,
			showCancelButton: show_cancel_btn,
			confirmButtonText: confirm_btn_txt,
			cancelButtonText: cancel_btn_txt,
			allowOutsideClick: outsite_clickable
		}).then((result) => {
			if (result.value) {
				this.approvedOrDisapprovedArticle(index);
			}
		});

	}

   /**
   * Clears all
   * purpose: to clear all from the search bar
   * created by: Momitha
   */
	clearAll() {
		this.search_text = '';
		this.pageOffset = 1;
		this.isPageSetFirstTime = false;
		this.articlesListng();
	}

  /**
   * Searchs users listing component
   * purpose: to search elements
   * craeted by: Momitha
   */
	search() {
		this.isPageSetFirstTime = false;
		this.pageOffset = 1;
		this.articlesListng();
	}



	/**
	 * Articles listng
	 * created by: Sourav
	 */
	articlesListng() {
		this.search_text = this.search_text ? this.search_text.trim() : '';
		let urlData = {
			url: 'admin/articles?page=' + this.pageOffset + '&limit=' + this.limitPerPage + '&search_text=' + this.search_text,
		};
		this.commonService.httpCallGet(urlData)
			.subscribe(data => {
				if (data.status == environment.HTTP_STATUS_OK) {
					this.articlesListDetails = data.data.rows;
					this.countTotalPage = data.data.count;
					if (!this.isPageSetFirstTime) {
						this.pager = this.pagerService.getPager(this.countTotalPage, this.pageOffset, this.limitPerPage);
						this.isPageSetFirstTime = true;
					}
				}
			});
	}

	/**
	 * Sets page
	 * @param page
	 * @returns
	 * created by Sourav
	 */
	setPage(page: number) {
		if (page < 1 || page > this.pager.totalPages) {
			return;
		}
		// get pager object from service
		this.pager = this.pagerService.getPager(this.countTotalPage, page, this.limitPerPage);
		// get current page of items
		this.pageOffset = page;
		this.articlesListng();
	}

	/**
	   * Actives or deactive user
	   * @param _index
	   * method: PUT
	   * input: @param _index
	   * output: data
	   * created by: Sourav
	   */
	approvedOrDisapprovedArticle(_index) {
		let status = 1;
		if (this.articlesListDetails[_index].is_approved == 1) {
			status = 0;
		}
		var updateUserStatusModel = {
			id: this.articlesListDetails[_index].id,
			is_approved: status,
		}

		var urldata = {
			url: 'admin/article-approve-status'
		}

		this.commonService.httpCallPut(urldata, updateUserStatusModel)
			.subscribe(data => {
				if (data.status == environment.HTTP_STATUS_OK) {
					this.commonService.commonToastrMessage(data.msg, 'success', 'Success');
					this.articlesListDetails[_index].is_approved = status;
				} else {
					this.commonService.commonToastrMessage(data.msg, 'error', 'Error');
				}
			});
	}


}
