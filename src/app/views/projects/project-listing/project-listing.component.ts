   ...
   */
	...
	changeProjectStatus(index, status) {
		...
		.subscribe(resp => {
			if (resp.status === environment.HTTP_STATUS_CREATED) {
				...
				this.projectList[index].status = project.status;
	...
	commonSwalAlert(title_txt: string, alert_type: any, show_cancel_btn: any, show_confirm_btn: any,
		...
		Swal.fire({
			...
			if (result.value) { this.commonService.log('status: ', status);
				if (status === '3') {
			...
			.subscribe(resp => {
				if (resp.status === environment.HTTP_STATUS_CREATED) {
					...
					this.notificationService.sendMessage({
						...
						"project_id"        : this.activeProjectId,
						"title"             : "Your project has been rejected by admin",
						"description"       : "",
						"title_arabic"      : "رفض المشرف مشروعك",
						"description_arabic": "",
						"type"              : "both"
					});
				} else {
