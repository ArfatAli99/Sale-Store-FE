...
export class UserDetailsComponent implements OnInit {
  ...
  contractorProfileUpdated: number = 0;
    ...
    this.commonService.httpCallPost(urlData, postData)
      .subscribe(data => {
        if (data.status == environment.HTTP_STATUS_OK) {
          ...
          this.contractorProfileUpdated = this.usersDetails.update;
          ...
          if (this.usersDetails.user_type == 3) {
            ...
            this.usersDetails.contractor_manpowers.map(itemVal => {
            ...
            this.usersDetails.contractor_metas.map(item => {
            ...
            // machineries
            let machineries = this.usersDetails.contractor_metas.filter(item => item.group_name == "machinery");
            ...
            // Client References
            let clientReferences = this.usersDetails.contractor_metas.filter(item => item.group_name == "client_reference_details");
            ...
            this.contractorResources = this.contractorResourceTypes.map(resource_type => {
              let resource = this.usersDetails.resources.find(resource => resource.type == resource_type);
...
*/
  ...
  /**
   * View contractor profile changes
   */
  viewProfileChanges() {
    this.router.navigate(['/users/profile-changes', this.route.snapshot.paramMap.get('id')]);
  }
