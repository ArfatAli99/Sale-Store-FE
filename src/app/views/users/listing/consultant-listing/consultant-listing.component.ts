...
@Component({
  selector: 'app-consultant-listing',
  templateUrl: './consultant-listing.component.html',
  styleUrls: ['./consultant-listing.component.scss']
})
export class ConsultantListingComponent implements OnInit {
  ...
  usersListDetails = [];
	...
	getUsersList() {
		...
		let urlData = {
			url: 'admin/consultant-list?' + this.commonService.convertToUrlParams(postData),
		};
	...
	*/
	sortUsers(sortBy) {
		...
		switch(this.activeSort) {
			...
			case 'project':
				this.activeSortText = this.activeSortBy == 'asc' ? 'projectAsc' : 'projectDsc';
				break;					
		}
